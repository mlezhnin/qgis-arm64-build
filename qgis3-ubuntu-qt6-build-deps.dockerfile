...
# Installs libspatialindex from source, 2.1 not supported on qgis because of
# issue: https://github.com/qgis/QGIS/issues/58485
RUN apt-get update && apt-get install -y wget build-essential cmake \
    ...
    && ldconfig \
    && cd ../.. \
    && rm -rf spatialindex-src-1.9.3 spatialindex-src-1.9.3.tar.gz
...
RUN apt-get update && apt-get install -y \
    ...
    libtiff-dev \
    && cd /tmp \
    && git clone --depth 1 --branch 2.8.4 https://github.com/PDAL/PDAL.git pdal-src \
    && cd pdal-src \
    && mkdir build && cd build \
    && cmake \
...
# Oracle : client side
RUN curl https://download.oracle.com/otn_software/linux/instantclient/2326000/instantclient-basic-linux.arm64-23.26.0.0.0.zip > instantclient-basic-linux.arm64-23.26.0.0.0.zip
RUN curl https://download.oracle.com/otn_software/linux/instantclient/2326000/instantclient-sdk-linux.arm64-23.26.0.0.0.zip > instantclient-sdk-linux.arm64-23.26.0.0.0.zip
RUN curl https://download.oracle.com/otn_software/linux/instantclient/2326000/instantclient-sqlplus-linux.arm64-23.26.0.0.0.zip > instantclient-sqlplus-linux.arm64-23.26.0.0.0.zip

RUN unzip -n instantclient-basic-linux.arm64-23.26.0.0.0.zip
RUN unzip -n instantclient-sdk-linux.arm64-23.26.0.0.0.zip
RUN unzip -n instantclient-sqlplus-linux.arm64-23.26.0.0.0.zip
...
RUN if [ -e /usr/lib/aarch64-linux-gnu/libaio.so.1t64 ] ; then ln -sf /usr/lib/aarch64-linux-gnu/libaio.so.1t64 /usr/lib/aarch64-linux-gnu/libaio.so.1 ; fi
...
# Uncomment to enable HANA client installation - requires accepting SAP's EULA
# NOTE: No ARM64 build available from SAP as of December 2025, so either privately host your own, or build one from source here
...
# Uncomment to enable MSSQL client installation - requires accepting MS's EULA
# NOTE: No ARM64 build available for msodbcsql18 and mssql-tools18 so either privately host your own, or build one from source here

...
FROM  my-qgis-deps:latest AS BUILDER
...
RUN SUCCESS=OK \
&& PATH="/usr/bin:/usr/local/bin:/bin" cmake \
-GNinja \
-DUSE_CCACHE=OFF \
...
RUN pip3 install --break-system-packages jinja2 pygments pexpect && apt-get install -y expect
